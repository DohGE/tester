    ...
    this.actions$.pipe(
      ...
      switchMap((action) =>
        this.menuService.saveExercise(action.exercise).pipe(
          ...
          catchError((error) => {
            this.notificationService.showError('Coś poszło nie tak', 'Error');
            return of(menuActions.saveExerciseFail({ error: error.error }));
          })
        )
      )
    )
    ...
    this.actions$.pipe(
      ...
      exhaustMap(() =>
        this.menuService.loadLessons().pipe(
          ...
          catchError((error) => {
            return of(menuActions.loadLessonsFail({ error: error.error }));
          })
        )
      )
    )
